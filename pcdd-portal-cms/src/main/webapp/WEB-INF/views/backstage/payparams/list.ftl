...
<html>
  ...
  <body>
  	<input type="hidden" id="queryUrl"  value="<@full_path path='backstage/payParams/?act=QUERY' />"  />
  	<input type="hidden" id="deleteUrl"  value="<@full_path path='backstage/payParams/?act=DELETE' />"  />
  	<input type="hidden" id="saveUrl"  value="<@full_path path='backstage/payParams/?act=SAVE' />"  />
  	...
  	<input type="hidden" id="fullPath"  value="<@full_path path='' />"  />
       ...
       <div class="container" style="width:100%;">
       	...
       	<!--新增或修改-->	
		<#include "/views/backstage/payparams/form.ftl"/>			
								
		 <div class="row">
				<!--栏目路径-->
						<div class="header">
	            <h1 class="page-title">商户支付管理</h1>
	          </div>
	          <ul class="breadcrumb">
	            <li><a href="javascript:void(0);">系统管理</a> <span class="divider">/</span></li>
	            <li class="active">商户支付管理</li>
            </ul>
		  </div>		  
					...
					<table class="table table-bordered table-hover" style="table-layout: fixed;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;">
						...
						<tbody data-bind="foreach: {data: model.dataList, as: 'item'}, visible: model.dataList().length" >
							<tr data-bind="css : { evenTr: $index()%2==1 }">
								...
								<td data-bind="text: item.mchType,click: $root.showForm.bind($root, item),attr:{title:item.mchType}"></td>
								<td data-bind="text: item.mchId,click: $root.showForm.bind($root, item),attr:{title:item.mchId}"></td>
								<td data-bind="text: item.mchKey,click: $root.showForm.bind($root, item),attr:{title:item.mchKey}"></td>
								<td >
...
<script type="text/javascript" src="<@full_path path='js/backstage/payparams/service.js'/>" ></script>	
<script type="text/javascript" src="<@full_path path='js/backstage/payparams/list.js'/>" ></script>	
  </body>
</html>

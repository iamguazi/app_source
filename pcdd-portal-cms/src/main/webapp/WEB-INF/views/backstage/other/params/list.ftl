...
<html>
  ...
  <body>
  	<input type="hidden" id="queryUrl"  value="<@full_path path='backstage/otherParams/?act=QUERY' />"  />
  	<input type="hidden" id="deleteUrl"  value="<@full_path path='backstage/otherParams/?act=DELETE' />"  />
  	<input type="hidden" id="saveUrl"  value="<@full_path path='backstage/otherParams/?act=SAVE' />"  />
  	...
  	<input type="hidden" id="fullPath"  value="<@full_path path='' />"  />
       ...
       <div class="container" style="width:100%;">
       	...
       	<!--新增或修改-->    
        <#include "/views/backstage/other/params/form.ftl"/>  
        
		
         <div class="row">
                <!--栏目路径-->
                        <div class="header">
                <h1 class="page-title">扩展参数列表</h1>
              </div>
              <ul class="breadcrumb">
                <li><a href="javascript:void(0);">扩展参数设置</a> <span class="divider">/</span></li>
                <li class="active">扩展参数列表</li>
            </ul>
          </div>   		  
					...
					<table class="table table-bordered table-hover">
						...
						<tbody data-bind="foreach: {data: model.dataList, as: 'item'}, visible: model.dataList().length" >
							<tr data-bind="css : { evenTr: $index()%2==1 }">
								...
								<td data-bind="text: item.paramsValue,click: $root.showForm.bind($root, item)" ></td>
								<td data-bind="text: item.paramsRemarks,click: $root.showForm.bind($root, item)" ></td>
								<td>
...
<script type="text/javascript" src="<@full_path path='js/backstage/other/params/service.js'/>" ></script>	
<script type="text/javascript" src="<@full_path path='js/backstage/other/params/list.js'/>" ></script>	
  </body>
</html>

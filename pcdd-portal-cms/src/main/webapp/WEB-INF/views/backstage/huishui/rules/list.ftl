...
<html>
  ...
  <body>
  	<input type="hidden" id="queryUrl"  value="<@full_path path='backstage/huiShuiRule/?act=QUERY' />"  />
  	<input type="hidden" id="deleteUrl"  value="<@full_path path='backstage/huiShuiRule/?act=DELETE' />"  />
  	<input type="hidden" id="saveUrl"  value="<@full_path path='backstage/huiShuiRule/?act=SAVE' />"  />
  	...
  	<input type="hidden" id="fullPath"  value="<@full_path path='' />"  />
       ...
       <div class="container" style="width:100%;">
       	...
       	<!--新增或修改-->	
		<#include "/views/backstage/huishui/rules/form.ftl"/>			
								
		 <div class="row">
				<!--栏目路径-->
						<div class="header">
	            <h1 class="page-title">回水规则设置</h1>
	          </div>
	          <ul class="breadcrumb">
	            <li><a href="javascript:void(0);">系统设置</a> <span class="divider">/</span></li>
	            <li class="active">回水规则设置</li>
            </ul>
		  </div>		  
		  ...
		  <div class="row" style="padding-left:.45em">
						...
						<button class="btn btn-danger btn-sm" data-toggle="modal" 
						   ...
						   data-bind="visible:model.checkItems().length,click:$root.showpop.bind($root,'delete','您确定要删除选中的回水吗？',$root.delItem.bind($root)) " >
					...
					<table class="table table-bordered table-hover">
						<thead>
							<tr>
								...
								<th width="10%">房间类型</th>
								<th width="10%">回水比例</th>
								<th width="20%">亏损起始积分</th>
								<th width="20%">亏损结束积分</th>
								<th width="25%">操作</th>
							</tr>
						</thead>
						<tbody data-bind="foreach: {data: model.dataList, as: 'item'}, visible: model.dataList().length" >
							<tr data-bind="css : { evenTr: $index()%2==1 }">
								...
								<td data-bind="text: $root.fmt.currency(item.bili,{symbol:''}),click: $root.showForm.bind($root, item)"></td>
								<td data-bind="text: $root.fmt.currency(item.startPoint,{symbol:''}),click: $root.showForm.bind($root, item)" ></td>
								<td data-bind="text: $root.fmt.currency(item.endPoint,{symbol:''}),click: $root.showForm.bind($root, item)" ></td>
								<td>
									<a href="javascritp:void(0);" data-bind="click:$root.showForm.bind($root, item)"><span class="glyphicon glyphicon-pencil"></span>编辑</a> 
									&nbsp; &nbsp; &nbsp;
									<a href="javascritp:void(0);" data-bind="click:$root.showpop.bind($root,'delete','您确定要删除该回水记录吗？',$root.delItem.bind($root,item.id),1)"><span class="glyphicon glyphicon-trash"></span>删除</a>
								</td>
							</tr>
						</tbody>
...
<script type="text/javascript" src="<@full_path path='js/backstage/huishui/rules/service.js'/>" ></script>	
<script type="text/javascript" src="<@full_path path='js/backstage/huishui/rules/list.js'/>" ></script>	
  </body>
</html>
